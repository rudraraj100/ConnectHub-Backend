...
/**
 * Auth Service Spring Security configuration.
 *
 * Responsibility:
 *  - Protect auth-service's own endpoints (second line of defence after gateway)
 *  - Drive the Google OAuth2 login flow (session-based state parameter)
 *  - JWT blacklist check via JwtAuthFilter + Redis
 *
 * CORS is NOT configured here — the API Gateway owns CORS exclusively via
 * spring.cloud.gateway.server.webmvc.globalcors in application.yaml.
 *
 * Session policy: IF_REQUIRED so OAuth2 state param survives the redirect.
 * STATELESS would break Google login (state mismatch error).
 */
@Configuration
@EnableWebSecurity
