...
public class Room {
    ...
    public void prePersist() {
        ...
        // Issue 6 fix: DM rooms must not get an inviteLink — joinByInviteLink() throws
        // 403 FORBIDDEN for DMs anyway, so the link is dead data that wastes the
        // unique index on invite_link and a UUID generation per DM creation.
        if (this.inviteLink == null && this.type != RoomType.DM) {
            this.inviteLink = UUID.randomUUID().toString().replace("-", "").substring(0, 12);
        }
