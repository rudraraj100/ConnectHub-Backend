...
public class NotifServiceImpl implements NotifService {
    ...
    public NotificationDTO send(SendNotificationRequest req) {
        ...
        // Push in-app via WebSocket to the recipient's personal topic
        ws.convertAndSend("/topic/notifications/" + req.getRecipientId(), dto);
        log.debug("Notification sent to {} via WS: {}", req.getRecipientId(), req.getTitle());
