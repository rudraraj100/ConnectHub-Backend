...
public class Message {
    ...
    /**
     * Sender's display name — stored at send-time from the frontend profile.
     * Nullable: historical messages before this field existed show null here,
     * which causes toResponse() to fall back to the auth-service Feign call.
     * New messages always have this populated so the Feign call is skipped.
     */
    ...
    /**
     * Room admin can pin one message per room (PREMIUM feature).
     * The room-service pin endpoint sets this flag; message-service
     * mirrors it so the history API can return isPinned=true on reload.
     */
